...
```
cd src/backend && .venv/bin/uvicorn main:app --host 0.0.0.0 --port 8000
cd src/frontend && npx vite preview --port 5180 --strictPort --host 127.0.0.1
```

Бекенд саме на `0.0.0.0`: QR паринга кличе телефон на LAN-адресу, і поки
uvicorn слухав `127.0.0.1`, телефону не було куди прийти — паринг «не працював»
саме через це, а не через протокол.
