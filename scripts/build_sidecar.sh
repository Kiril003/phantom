...
#
# Четверте сімейство — випікання дорожніх пакетів. `osmium` імпортується
# всередині функцій робітника (`geo/bake/worker.py`), тож у графі імпортів
# його немає; а без нього пакунок збереться, підніметься й відмовить лише
# тоді, коли людина натисне «спекти».
for pkg in torch sentence_transformers transformers tokenizers safetensors \
           ...
           pypdf openpyxl striprtf osmium \
...
# ЖОРСТКА ВІДМОВА, а не рядок у журналі.
#
# Тут стояло `say "не знайдено, пропущено: …"` — і це були ворота, які НЕ
# ВМІЮТЬ ПОЧЕРВОНІТИ. Кожне ім'я в списку вище потрапило туди тому, що без
# нього пакунок мовчки калічний: без aiosqlite вузол помирає на першому
# запиті до бази, без pypdf модуль «чесно відмовляє» і мертвий бандл
# виглядає як продукт, що просто не вміє PDF, без osmium не печеться жоден
# пакет. Тобто список — це перелік речей, відсутність яких НЕ ВИДНО на
# збірці й видно лише в людини. Попередження в журналі на 400 рядків
# виводу не бачить ніхто, а зелена збірка — бачать усі.
#
# Якщо пакет справді став не потрібен, його прибирають зі списку — свідомо,
# однією правкою. Мовчки випасти він більше не може.
if [ -n "${SKIPPED_PKGS}" ]; then
  echo "[сайдкар] НЕМАЄ в середовищі збірки:${SKIPPED_PKGS}"
  echo "[сайдкар] Кожне з цих імен — те, чия відсутність не видно на збірці"
  echo "[сайдкар] і видно лише в користувача. Постав їх у ${BUILD_VENV}"
  echo "[сайдкар] (перевір requirements-bundle.txt) або прибери зі списку свідомо."
  exit 1
fi
