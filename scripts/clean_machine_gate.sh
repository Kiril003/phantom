...
run_image() {
  ...
  # Скільки взагалі приїхало вимірів. Нуль означає, що контейнер НЕ ЗАПУСКАВСЯ
  # — і це не те саме, що «пакунок не працює». 04.09 чужа перевірка тримала
  # спільний замок, `flock -w` його не дочекався, docker не стартував жодного
  # разу, а ворота написали «ПАКУНОК НЕ РОЗПАКУВАВСЯ: без подробиць» на всіх
  # трьох образах. Червоне з чужою причиною гірше за відсутність червоного:
  # воно посилає шукати ваду туди, де її немає.
  R[KVCOUNT]="${kvn}"
  R[RC]="${rc}"
  R[IMAGE]="${img}"
  R[SKIP_TLS]="${skip_tls}"
...
verdict() {
  ...
  # Контейнер не запускався взагалі — вимір НЕ ВІДБУВСЯ. Найчастіша причина
  # на спільній машині: замок тримає чужа перевірка довше, ніж наш `-w`.
  if [ "${R[KVCOUNT]:-0}" = "0" ]; then
    printf '  \033[31mЧЕРВОНЕ\033[0m — %s\n' "ВИМІР НЕ ВІДБУВСЯ: контейнер не стартував (rc=${R[RC]:-?}), жодного виміру не приїхало"
    say "  найімовірніше — замок ${LOCK} тримала чужа перевірка довше за --lock-wait ${LOCK_WAIT} с"
    say "  це НЕ вирок пакунку: про нього тут не сказано нічого"
    RES_LINES+=("ЧЕРВОНЕ ${img}  вимір не відбувся: контейнер не стартував (замок/docker), про пакунок не сказано нічого")
    return 1
  fi
