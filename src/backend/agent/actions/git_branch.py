...
class GitBranchManage(Action):
    ...
    async def execute(self, ctx: ActionContext) -> ActionResult:
        ...
        try:
            if self.op == "list":
                res = await asyncio.to_thread(subprocess.run, ["git", "branch"], cwd=cwd, capture_output=True, text=True)
            ...
            if self.op == "create":
                ...
                check = await asyncio.to_thread(subprocess.run, ["git", "rev-parse", "--verify", self.branch_name], cwd=cwd, capture_output=True)
                if check.returncode == 0:
                    return ActionResult(ok=False, error=f"branch {self.branch_name} already exists", error_class="already_exists")

                await asyncio.to_thread(subprocess.run, ["git", "checkout", "-b", self.branch_name, self.base_branch], cwd=cwd, check=True)
            ...
            if self.op == "switch":
                await asyncio.to_thread(subprocess.run, ["git", "checkout", self.branch_name], cwd=cwd, check=True)
                return ActionResult(ok=True, output=f"Switched to branch {self.branch_name}")

            if self.op == "merge":
                # Switch to base first
                await asyncio.to_thread(subprocess.run, ["git", "checkout", self.base_branch], cwd=cwd, check=True)
                res = await asyncio.to_thread(subprocess.run, ["git", "merge", self.branch_name], cwd=cwd, capture_output=True, text=True)
                if res.returncode != 0:
            ...
            if self.op == "delete":
                await asyncio.to_thread(subprocess.run, ["git", "branch", "-D", self.branch_name], cwd=cwd, check=True)
