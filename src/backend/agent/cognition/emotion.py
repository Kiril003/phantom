...
async def update_emotion_on_event(
    ...
    # Phase 13 — Sentient Endocrine Bridge
    try:
        from ai.sentience.endocrine import endocrine_system
        if key == "task.failed":
            endocrine_system.stimulus(cortisol_delta=0.15, dopamine_delta=-0.1)
        elif key == "user.praise":
            endocrine_system.stimulus(oxytocin_delta=0.1, dopamine_delta=0.2, cortisol_delta=-0.1)
        elif key == "user.interrupted":
            endocrine_system.stimulus(cortisol_delta=0.1)
    except Exception as exc:
        logger.debug("endocrine stimulus hook failed: %s", exc)
