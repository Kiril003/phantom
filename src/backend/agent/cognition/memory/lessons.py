...
Стисло. Прескриптивно. Без переказу подій. 

Якщо результат SUCCESS (done): перекладай "що працювало" як готову інструкцію.
Якщо результат FAILURE/STOPPED: зосередься на "чого уникати" (what_avoid). 
Поясни, який шлях виявився тупиковим або чому виник цикл.
...
Поверни строго JSON:
{{
  "what_worked": "<≤200 символів — інструкція що робити (для успіху)>",
  "what_avoid": "<≤200 символів — попередження, чому НЕ робити (особливо для провалу)>",
