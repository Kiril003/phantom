...
def _query_sync(query: str, k: int) -> list[dict[str, Any]]:
...
async def recall(query: str, k: int | None = None) -> list[dict[str, Any]]:
    ...
    return await asyncio.to_thread(_query_sync, query, max(1, int(limit)))
