...
def _browser_page_active(value: Any) -> bool:
    ...
    from agent.kernel.runtime import agent_runtime  # local import to avoid cycle
    browser = getattr(agent_runtime, "browser", None)
    page = getattr(agent_runtime, "browser_page", None)
    return bool(browser and page)


def _port_free(value: Any) -> bool:
    """Check if a TCP port is currently free on localhost."""
    try:
        port = int(value)
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            s.settimeout(0.5)
            # If connect_ex returns 0, the port IS in use. We want it NOT in use.
            return s.connect_ex(("127.0.0.1", port)) != 0
    except (ValueError, TypeError, OSError):
        return False


def _ram_available(value: Any) -> bool:
    """Check if at least `value` MB of RAM is available."""
    try:
        required_mb = int(value)
        from core.system_monitor import system_monitor
        snap = system_monitor.current()
        if snap:
            return snap.ram_available_mb >= required_mb
        return True # Fallback if monitor isn't running
    except Exception:
        return True
...
PRECONDITION_REGISTRY: dict[str, CheckFn] = {
    ...
    "port.free": _port_free,
    "system.ram_available": _ram_available,
}
