...
ПАТЕРНИ ВІДНОВЛЕННЯ (застосовуй при повторі):
- Якщо попередня дія завершилась 'selector_no_match' або подібним: НЕ повторюй
  той самий selector. Спробуй browser.click_by_description (візуальне
  розпізнавання) або інший, структурно відмінний selector. Якщо не знаєш який
  елемент потрібен — використай browser.extract з більш загальним selector
  (наприклад 'body' або 'main') щоб побачити структуру сторінки.
- Якщо повторна спроба тієї ж дії з тими ж аргументами дала той самий результат:
  зміни підхід повністю, не просто аргументи.
- Якщо дію 3+ разів виконано без прогресу до sub_goal: поверни DONE_SUBGOAL з
  поясненням чому ціль недосяжна в поточних умовах.
- Якщо спостереження містить тег 'hint:browser_reset_after_resume': попередня
  сесія браузера втрачена після рестарту/відновлення. Якщо сюжет потребує
  конкретної сторінки — першою дією зроби browser.navigate до того URL, який
  згаданий у спостереженнях. Не очікуй, що раніше відкрита сторінка ще там.
"""
