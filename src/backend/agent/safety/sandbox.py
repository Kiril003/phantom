...
def _profile_flags(profile: SandboxProfile, workspace_dir: str | None) -> list[str]:
    ...
    # Network gating.
    if profile is SandboxProfile.net_observe:
...
def wrap_argv(
    ...
    *,
    ...
    tells the truth (`agent.audit.ActionResult.sandboxed`).
    ...
    if not bwrap_available():
        logger.warning(
            "agent.safety.sandbox: bwrap missing on PATH — running %s "
    ...
    if memory_limit_bytes is not None and memory_limit_bytes > 0:
        ...
        # the only loss.
        logger.debug(
            "agent.safety.sandbox: prlimit missing — proceeding without "
