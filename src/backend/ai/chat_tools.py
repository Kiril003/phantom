...
CHAT_DATA_TOOLS: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "description": (
            ...
            "Агент УМІЄ ШУКАТИ В МЕРЕЖІ без жодного ключа (DuckDuckGo) — тож "
            "дослідницьку мету делегуй сюди, навіть якщо хмарний пошук недоступний. "
    ...
    },
    {
        "name": "read_file",
        "description": (
            "Прочитати файл усередині дозволеного дерева. Текст приходить як "
            "kind='text' з content — до 1 МіБ, далі truncated=True. Документ "
            "(за розширенням: PDF, DOCX, XLSX, PPTX, RTF, ODT, ODS) "
            "розбирається в текст на пристрої й приходить як kind='document' "
            "з text: файл до 16 МіБ, тіло тексту до 200 000 символів. Не "
            "влізло — truncated=True, поле truncation і рядок у кінці самого "
            "тексту: ти бачив лише початок. "
            "Усе інше — kind='unreadable' з reason: двійкове до тебе не "
            "їде взагалі, base64 у відповіді не буває ніколи. На 'unreadable' "
            "і на truncated=True скажи людині словами, чого саме не прочитав, "
            "і назви причину — переказувати вміст документа чи зображення, "
            "якого не бачив, заборонено."
        ),
        "parameters": {
            "type": "object",
