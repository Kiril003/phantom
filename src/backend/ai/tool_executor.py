...
def _document_text_or_refusal(
    ...
    if cut:
        # Число беремо з ТОГО, ЩО СПРАВДІ ПОЇХАЛО, а не зі стелі: різав або
        # розбирач (на своїй межі), або я (на своїй), і сказати треба факт.
        out["truncation"] = (
            f"показано {len(text)} символів — кінець документа сюди не доїхав, "
            "і що в ньому, я не знаю"
        )
        out["say_it"] = (
