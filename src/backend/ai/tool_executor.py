...
# Запас під рядок розбирача про обрив, який стоїть у КІНЦІ тексту — щоб він
# доїхав цілим, а не потрапив під ніж разом із хвостом тіла.
_TRUNCATION_NOTE_SLACK: int = 512
...
def _document_text_or_refusal(
    ...
    # Ніж падає НЕ на самій стелі: розбирач дописує в кінець рядок «показано
    # N символів із M — далі обрізано», і він сильніший за моє сусіднє поле,
    # бо модель читає його в тому самому рядку, що й зміст, і бачить справжнє
    # M. Запас — рівно на нього. Але запас не дірка: усе, що вилізе за нього
    # (напр. розбирач, який знехтував `max_chars`), однаково ріжеться.
    if len(text) > DOC_TEXT_MAX_CHARS + _TRUNCATION_NOTE_SLACK:
    ...
    if cut:
        # Числа тут НЕМАЄ навмисно, і це не лінь. `len(text)` рахує й
        # примітку розбирача про обрив (83 символи), тобто назвати його
        # «показано N символів документа» означало б порахувати слова ПРО
        # обрізання як обрізаний текст — той самий клас дефекту, проти якого
        # стоїть увесь цей стик. Скільки байтів поїхало — каже `chars`, і це
        # точно. Скільки з них зміст документа — знає лише той, хто різав, і
        # він пише це в кінці самого тексту.
        # Показувати пальцем на примітку розбирача теж не можна: коли стелею
        # знехтували і різав Я, її в тексті немає взагалі.
        out["truncation"] = (
            "текст обрізано: кінець документа сюди не доїхав, і що в ньому, я не знаю"
        )
        out["say_it"] = (
