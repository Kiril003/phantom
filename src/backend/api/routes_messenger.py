...
async def add_contact(
    ...
    # Запрошення З ТЕЛЕФОНА — інший формат (PH2), і декодера для нього в
    # цьому вузлі немає взагалі. Доти людина вставляла код зі свого телефона
    # й діставала «неприйнятний bundle» — відмову, яка описує НАШ розбір, а
    # не її дію. Правда проста: телефон додається не запрошенням, а
    # спаруванням, і код для цього показує сам ПК. Мосту форматів тут
    # свідомо не будуємо — просто перестаємо мовчати не про те.
    for candidate in (payload.compact, payload.peer_address):
        if candidate and candidate.strip().upper().startswith("PH2:"):
            raise HTTPException(
                status_code=400,
                detail=(
                    "це запрошення з телефона (PH2). Телефон додається "
                    "спаруванням: відкрий «Пристрої» на цьому компʼютері й "
                    "проскануй телефоном код з екрана — вставляти сюди його "
                    "запрошення не треба."
                ),
            )
