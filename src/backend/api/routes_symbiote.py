...
async def _learn_from_phone_radio(
    ...
    try:
        ...
        # Модуль перенесено в це дерево 30.08 — тепер ця гілка не має
        # спрацьовувати ніколи. Але лишаємо її як заслін: якщо файл колись
        # знову зникне при зшиванні гілок, телефонний виклик не має
        # 500-ити через фонове донавчання радіо.
        #
        # Рівень підняв із debug до **warning** свідомо. Раніше це стояло
        # в debug: телефон робив справжній скан (до 24 точок), слав його
        # сюди, а ПК мовчки викидав — і побачити втрату можна було лише
        # ввімкнувши debug-лог, тобто практично ніколи. Мовчазне
        # викидання чужої роботи не має права бути тихішим за попередження.
        logger.warning(
            "symbiote: geo.wifi_scan відсутній — пакет радіо від %s ВИКИНУТО, "
            "донавчання не відбулось", device.id,
        )
        return
