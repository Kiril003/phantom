...
class PhantomConfig(BaseSettings):
    ...
    # ── Server ────────────────────────────────────────────────────────────────
    # Ф0 (master-plan §5): loopback за замовчуванням. Головний порт віддає
    # чат, /linux, голос і файли БЕЗ TLS — на 0.0.0.0 це подарунок будь-кому
    # в тій самій Wi-Fi. LAN-шляхи мають власні явні двері: телефони ходять
    # через TLS-слухач (pair_tls_port, сам сідає на LAN-адреси), mDNS сам
    # оголошує інтерфейси, а оператор, якому свідомо треба відкритий HTTP,
    ...
    #: Чи вільно ПАКОВАНІЙ збірці відчиняти двері в локальну мережу.
    #:
    #: Заміряно 12.09.2026 на живому пакунку: `158.196.239.126:8443` слухав на
    #: `wlan0` у публічній /21 — і ніхто про це не питав. Сторож
    #: `_refuse_lan_bind_in_packaged_mode` тут не допомагав: він судить
    #: `config.host`, тобто головний HTTP-порт, а TLS-слухач і mDNS ідуть
    #: ВЛАСНИМИ дорогами, повз нього. Це «один if — двоє дверей», тільки
    #: другі двері ширші.
    #:
    #: У пакунку двері в мережу мають відчинятись ЗА ЗГОДОЮ людини, а не самі.
    #: У розробці (без `PHANTOM_PACKAGED=1`) нічого не міняється: там ці двері
    #: і є робочим інструментом.
    lan_doors_enabled: bool = False
    # Відбиток сертифіката для QR, коли перед вузлом стоїть чужий TLS (Caddy,
    # mkcert) і власного сертифіката вузол не бачить. `routes_pair` читав це
    # поле й описував його в коментарі як обов'язкове для продакшену, а поля
    # не існувало: PAIR_CERT_SHA256 мовчки ігнорувався, у QR їхало «dev-no-pin»,
    # і телефон вимикав пінінг саме в тому розгортанні, заради якого це писали.
    pair_cert_sha256: str = ""
    ...
    relay_url: str = ""
    # Сховок PH5 — ОКРЕМА дорога від relay_url вище, і ключ окремий навмисно.
    #
    # relay_url веде до WS-тунелю (`/relay/node`), якого на розгорнутому
    # сервері не існує: заміряно 29.08 — 404, тоді як /relay/drop|fetch|burn
    # відповідають. Вписати сюди адресу сховка означало б розбудити
    # `start_relay_client` (relay_client.py:277) і приректи вузол вічно
    # стукати в неіснуючий шлях із відкотом до 60 с.
    #
    # Тут же — та сама мова, якою говорить телефон (`PeerRelayClient.kt`):
    # сервер тримає конверт сталої довжини під 43-символьною адресою і не
    # знає ні від кого він, ні кому. Тому типове значення тут Є: сервер
    # розгорнуто, і вузол, який до нього не ходить, просто не отримує пошти.
    relay_store_enabled: bool = True
    relay_store_url: str = "https://phantom-license.fly.dev"
    ...
    #
    # Випікання — найважче, що продукт робить із памʼяттю, і воно триває
    # хвилини. Тому в нього є власний запобіжник: поки вільної памʼяті менше
    # за `bake_min_ram_pct`, робітник пригальмовує, а після
    # `bake_low_strikes` поспіль вимірів нижче порога — здається й каже про
    # це, замість того щоб чекати пострілу від oom-guard.
    #
    # ПІДЛОГА 15% І ЧОМУ САМЕ ВОНА. Демон oom-guard на машині власника б'є
    # на 12% вільної памʼяті й обирає жертву за розміром — тобто під час
    # випікання жертвою є рівно робітник. Дозволити тут число нижче 12
    # означало б написати налаштування, яке НЕ МОЖЕ спрацювати: система
    # застрелила б робітника раніше, ніж наш поріг узагалі був би
    # перевірений, а людина бачила б «сама впала» без причини. Запас у три
    # пункти лишає нам час зупинитись самим і сказати чому.
    bake_min_ram_pct: int = Field(default=30, ge=15, le=90)
    bake_low_strikes: int = Field(default=3, ge=1, le=20)
    # Витяг OSM важить сотні мегабайтів і качається хвилинами. Лишаємо його
    # на диску за замовчуванням: друге випікання того ж обсягу (інші
    # налаштування, новіший код) тоді не платить за мережу вдруге. Кому
    # місце дорожче за час — вимикає, і тоді витяг гине разом із роботою.
    bake_keep_source_extracts: bool = True
