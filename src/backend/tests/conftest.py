...
# ── Хто саме власник у цьому стенді ───────────────────────────────────────────


class NoStandIdentity(AssertionError):
    """Стенд не має особи, від імені якої діє."""


def owner_of(client) -> str:
    """Ідентифікатор користувача, ЯКИМ АВТЕНТИФІКОВАНИЙ клієнт.

    Навіщо окремий помічник. Двічі за добу тест брав власника як «першого
    користувача бази» (`select(User.id).order_by(User.id)` → `.first()`).
    Ідентифікатори — випадкові uuid, а користувачів у базі більше одного, тож
    приблизно кожен третій прогін давав ІНШОГО власника: рядки народжувались
    під чужим власником, вузол чесно відповідав «не знайдено», і падав тест, а
    не продукт. Другого разу та сама причина ще й лишила два сусідні тести
    ЗЕЛЕНИМИ вхолосту — вони стверджували «порожньо», а порожньо було саме
    тому, що нічого не доїхало.

    Тому помічник **відмовляється** вгадувати. Немає токена — виняток, а не
    «перший-ліпший рядок»: там, де можна прибрати вибір, на дисципліну не
    покладаємось.
    """
    from jose import jwt as _jwt

    header = getattr(client, "headers", {}).get("Authorization", "")
    if not header.startswith("Bearer "):
        raise NoStandIdentity(
            "клієнт стенда без Authorization: власника беруть із токена, "
            "а не з першого рядка таблиці User"
        )
    claims = _jwt.get_unverified_claims(header.split(" ", 1)[1])
    subject = str(claims.get("sub") or "")
    if not subject:
        raise NoStandIdentity("у токені немає `sub` — особи стенда не існує")
    return subject
