...
def test_обрізання_каже_себе_вголос(tmp_path):
    ...
    assert ціле.ok and len(ціле.text) > 20
    res = document_text.extract(d, max_chars=20)
    assert res.ok, res.reason
    assert "обріз" in res.text, f"обрізано мовчки: {res.text!r}"
    assert str(len(ціле.text)) in res.text, "не сказано, скільки символів було"
