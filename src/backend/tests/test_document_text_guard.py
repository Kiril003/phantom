...
# Рядки, які написав про себе САМ модуль. Виняток НАВМИСНО вузький і
# прив'язаний до ЦІЛОГО рядка. Широке правило «усе, що починається з # або
# [» знімало зі сторожа рядок кошторису, чия ПЕРША колонка — формула без
# збереженого значення (`[формула =A1*2 …]\t102000`), а разом із ним і всю
# колонку сум. Тобто сам сторож мав сліпу зону рівно там, де рахують гроші,
# і жоден тест не міг би це показати — зелене, яке не вміє почервоніти.
_ВЛАСНА_ПОЗНАЧКА = re.compile(
    r"# слайд \d+"  # номер показу — число модуля, не документа
    r"|# аркуш: .*"  # назва аркуша лежить в атрибуті XML, а не в тексті
    r"|# (?:колонтитул|виноски|нотатки доповідача)"
    r"|\[показано \d+ символів.*"  # примітка про обрізання
)


def _output_numbers(text: str) -> set[str]:
    """Числа з витягу, окрім рядків, які модуль написав про себе САМ.

    Питати джерело з «# слайд 3» безглуздо: це номер показу, і його вигадав
    модуль. Але кожен рядок, який модуль не склав ЦІЛКОМ, лишається під
    сторожем — включно з тим, що починається з `[формула …]`.
    """
    body = [ln for ln in text.split("\n") if not _ВЛАСНА_ПОЗНАЧКА.fullmatch(ln)]
...
def test_виняток_сторожа_не_ховає_рядків_документа():
    """Карвіут — теж місце, де зелене може розучитись червоніти.

    Перевіряється не формулювання позначок, а МЕЖА винятку: рядок, який
    модуль не склав цілком, мусить лишатись під наглядом, хоч би з чого він
    починався.
    """
    рядок = "[формула =A1*2 без збереженого значення]\t4800072000"
    assert "4800072000" in _output_numbers(рядок), "рядок кошторису випав зі сторожа"
    assert "4800072000" in _output_numbers("# 4800072000 позицій"), "абзац із решіткою випав"
    # А власні позначки модуля справді лишаються поза сторожем.
    assert _output_numbers("# слайд 3") == set()
    assert _output_numbers("# аркуш: 2026") == set()
    assert _output_numbers("[показано 20 символів із 28 — далі обрізано]") == set()
...
def test_формула_без_кешу_каже_це_а_не_віддає_порожнечу(tmp_path):
    ...
    # Рядок із формулою — теж рядок документа: числа в ньому мусять мати джерело.
    invented = _output_numbers(res.text) - _source_numbers(x)
    assert not invented, f"у витягу числа, яких немає в книзі: {sorted(invented)}"
