...
class TestGeminiToolUse:
    ...
    async def test_text_only_reply_comes_back_as_text_not_as_a_nameless_tool(
        ...
        Контракт лишено чинним свідомо (рішення штабу 03.09.2026, після того
        як я винесла вибір нагору замість тихої правки): повертати
        MODEL_REFUSED після трьох спроб означало б міняти І продукт, І його
        споживача заради того, щоб зелений став старий тест. Ціна не та —
        особливо перед релізом. Тому змінено сторожа, і причина записана
        тут, а не лише в повідомленні коміту, який ніхто не читає через
        півроку.
        """
        from ai import gemini_provider
        from ai.tool_use import ToolCallResult, all_tactical_tools
