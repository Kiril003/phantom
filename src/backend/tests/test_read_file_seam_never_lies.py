...
async def test_справжня_примітка_розбирача_вміщується_в_запас(tmp_path):
    ...
    («хвіст понад стелю 84 символи проти запасу 8»), а зрізаний хвіст ловить
    друге твердження.

    Заміряно 03.09.2026, і кожен доданок названо, бо порівнюється саме сума:
    84 = 81 (примітка розбирача) + 2 (роздільник) + 1 (мій пробний символ,
    бо прошу стелю+1). Тіло після `rstrip` = рівно 200 001, тож розбирач не
    з'їв нічого зайвого. Запас міряється ПОНАД СТЕЛЮ, а не понад попрошене,
    бо саме з цією сумою його порівнює ніж.
    """
    ...
    assert overhang <= SLACK, (
        f"хвіст понад стелю виріс до {overhang} символів проти запасу {SLACK} — "
