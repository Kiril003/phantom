...
await a.screenshot({ path: `${OUT}/r2_1_A_before.png` });
...
await b.screenshot({ path: `${OUT}/r2_2_B_before.png` });
...
await a.screenshot({ path: `${OUT}/r2_3_A_menu.png` });
...
await a.screenshot({ path: `${OUT}/r2_4_A_modal.png` });
...
report.a_tombstone_text = report.a_after_tombstones
  ...
  : '(немає)';
await a.screenshot({ path: `${OUT}/r2_5_A_tombstone.png` });

// ── Що тепер бачить B ────────────────────────────────────────────────────────
await b.reload({ waitUntil: 'networkidle' });
await b.waitForTimeout(6000);
await openChat(b, 'А');
await b.evaluate(() => { const d=[...document.querySelectorAll('div')].filter(e=>e.scrollHeight>e.clientHeight+200).pop(); if(d) d.scrollTop=d.scrollHeight; });
...
report.b_tombstone_text = report.b_after_tombstones
  ...
  : '(немає)';
await b.screenshot({ path: `${OUT}/r2_6_B_tombstone.png` });
