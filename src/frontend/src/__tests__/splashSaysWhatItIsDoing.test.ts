...
describe('слова заставки', () => {
  ...
  it('перший стан називає роботу й номер кроку, а не очікування', async () => {
    ...
    // Що саме робиться — людськими словами. Не «розпаковую»: onedir не
    // розпаковує нічого, і текст мусить бути правдивим для обох розкладок.
    expect(shown()).toMatch(/запускаюсь/);
    expect(shown()).not.toMatch(/розпаков/);
