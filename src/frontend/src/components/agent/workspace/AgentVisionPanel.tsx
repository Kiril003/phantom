...
export function AgentVisionPanel({ open, onClose }: Props) {
  ...
  useEffect(() => {
    ...
    // Phase 32-VISION: Subscribe to binary vision stream
    wsClient.send({ control: 'subscribe', channels: ['vision'] });

    const handleVision = (msg: any) => {
      if (msg.type === 'frame' && msg.data) {
        setFrame({
          width: msg.data.width,
          height: msg.data.height,
          strategy: msg.data.strategy,
          png_base64: msg.data.png_base64,
        });
        setError(null);
        setTried(null);
      }
    ...
    wsClient.on('vision', handleVision);

    return () => {
      wsClient.off('vision', handleVision);
      wsClient.send({ control: 'unsubscribe', channels: ['vision'] });
    };
  }, [open, paused]);
              ...
              {dataUrl ? (
                ...
                >
                  ...
                  <div style={{ fontSize: 14, marginBottom: 8 }}>
                    {error ? 'Бекенд скріншоту недоступний' : 'Очікую перший кадр (Live Stream)…'}
                  </div>
                  {error && (
                    <div
                      style={{
