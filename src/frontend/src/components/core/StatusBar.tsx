...
/**
 * Піч дорожніх пакетів видно з будь-якого столу — бо випікання країни
 * триває довше, ніж людина сидить на мапі.
 *
 * Головне тут: закінчена робота НЕ зникає, як зникає тост. Значок стоїть,
 * поки людина не відкрила її й не натиснула «До вибору обсягу»
 * (`bakeStore.acknowledge()`), і переживає перезавантаження — бо стан
 * читається з бекенда (`/bake/jobs/last`), а не з памʼяті вкладки.
 * Відсотка тут немає: у випікання немає знаменника, а показувати відсоток
 * завантаження як відсоток усієї роботи означало б збрехати вдвічі.
 */
/* BakeBadge жив тут і був НЕДОСЯЖНИЙ: `StatusBar` не монтується на шляху
   стола (`layouts/DashboardLayout.tsx`), тож його не бачив ніхто. Переїхав
   у стрічку організму — `components/organism/pulses.ts:useBakePulse`. */
