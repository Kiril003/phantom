...
export function DialogueSourceChip(): JSX.Element {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      if (fbReady && fallback) {
        // Кажемо НАСЛІДОК, не стан: «модель не в памʼяті» — правда про
        // рантайм, «перша відповідь довша» — правда про те, що станеться з
        // людиною. Поле `ai_fallback_model_loaded` дала Сесія 5 саме під це:
        // хост ollama живий, але моделі в памʼяті немає, тож перший лист
        // підіймає 4,7 ГБ і коштує десятки секунд.
        const cold = fbLoaded === false ? ' · перша відповідь довша' : '';
        setState({
          kind: 'named',
