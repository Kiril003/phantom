  ...
  return (
    ...
    >
      ...
      <div className="space-y-1.5">
        <div className="flex items-center justify-between text-[10px] text-[color:var(--ink-muted)] uppercase tracking-widest">
      ...
      <div className="max-h-[240px] overflow-y-auto space-y-2 pr-1 custom-scrollbar">
        {regions.length === 0 && !loading && (
          <div className="py-8 text-center text-[color:var(--ink-muted)] italic text-xs">
        ...
        {regions.map((r) => (
          <div
            key={r.id}
            className="group flex items-center justify-between p-2.5 rounded-xl bg-black/[0.04] border border-black/5 hover:border-white/15 transition-all"
          >
            <div className="flex flex-col gap-0.5">
              <span className="text-[11px] font-medium text-[color:var(--ink-primary)]">{r.name}</span>
              <div className="flex items-center gap-2 text-[9px] text-[color:var(--ink-muted)] uppercase tracking-tighter">
            ...
            <button
              ...
              className="p-2 text-[color:var(--ink-muted)] hover:text-rose-600 hover:bg-rose-500/10 rounded-lg transition-all opacity-0 group-hover:opacity-100"
            >
              <Trash2 size={14} />
            </button>
          </div>
      ...
      <button
        className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl bg-amber-500/20 hover:bg-amber-500/30 border border-amber-500/30 text-amber-700 text-[11px] font-semibold uppercase tracking-wider transition-all active:scale-[0.98]"
      >
        <Download size={14} />
        Завантажити область
      </button>
