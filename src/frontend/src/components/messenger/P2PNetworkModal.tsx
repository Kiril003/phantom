...
export const P2PNetworkModal: React.FC<P2PNetworkModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/75 backdrop-blur-md flex items-end sm:items-center justify-center p-0 sm:p-4 select-none animate-in fade-in duration-150">
      <div className="bg-[#121A15] border-t sm:border border-[#2B3C30] rounded-t-3xl sm:rounded-3xl w-full max-w-2xl shadow-2xl overflow-hidden flex flex-col max-h-[92dvh] sm:max-h-[90vh] animate-in slide-in-from-bottom sm:zoom-in-95 duration-150 pb-[var(--sab)] sm:pb-0 text-[#E4EDE7]">
        ...
        {/* Modal Top Header */}
        <div className="px-4 sm:px-5 py-3 sm:py-4 bg-[#141C16] border-b border-[#1F2B22] flex items-center justify-between shrink-0">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-2xl bg-[#1A261D] border border-[#2B3E31] flex items-center justify-center text-[#55C778] shadow-sm">
              <Radio className="w-5 h-5 animate-pulse" />
            </div>
            <div>
              <div className="flex items-center gap-2">
                <h3 className="font-extrabold text-base text-white">Керування протоколами зв'язку</h3>
                <span className={`px-2 py-0.5 rounded-full text-[10.5px] font-bold border flex items-center gap-1.5 ${
                  ...
                  <span className={`w-2 h-2 rounded-full ${
                    diagnostics.activeStatus === 'p2p-direct' ? 'bg-[#55C778]' : diagnostics.activeStatus === 'server-ws' ? 'bg-[#3B82F6]' : 'bg-[#F59E0B]'
                  }`} />
                  <span>
                    {diagnostics.activeStatus === 'p2p-direct'
                      ? 'P2P Direct [WebRTC]'
