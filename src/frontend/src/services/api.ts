...
export const authApi = {
   ...
   * `phantom` сама і мінтить йому випадковий шестизначний PIN. Той PIN лягає
   * у файл `identity/bootstrap_pin` (на Windows —
   * `%LOCALAPPDATA%\PHANTOM-OS\PHANTOM\…`) і друкується в журнал запуску.
   * У портативного застосунку немає ані консолі, ані причини знати цей шлях,
   * а автовхід свідомо вимкнений, доки PIN бутстрапний. Тобто людина, яка
   * щойно розпакувала архів, бачить запит PIN, якого для неї не існує ніде.
   * Це не «забув пароль» — це замок без ключа з коробки.
   *
   * Обидві ручки живуть лише на петлі й лише поки вузол нічий; після
   * [claimBootstrap] двері зачинені назавжди (409). Сам PIN звідси НЕ
   * повертається: секрет, якого не віддають, не можна ані підслухати, ані
   * лишити в журналі проксі.
   */
  bootstrapState: () => request<{ unclaimed: boolean }>('GET', '/auth/bootstrap'),

  /** Перший запуск: людина називає себе і ставить СВІЙ PIN. */
  claimBootstrap: (displayName: string, pin: string) =>
    request<AuthResponse>('POST', '/auth/bootstrap/claim', {
