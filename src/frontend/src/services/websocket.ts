...
export type WSChannel =
  ...
  | 'call'
  // Піч дорожніх пакетів — `job.started` / `job.progress` (не частіше разу
  // на секунду) / `job.stage` / `job.finished` / `pack.visible`. У `data`
  // лежить ПОВНИЙ знімок роботи, тож приймач заміняє свій стан, а не
  // зливає. Випікання країни триває довше за життя вкладки, тому цей
  // канал — лише пришвидшувач: правда завжди читається з
  // `GET /bake/jobs/current`, і саме тому перезавантаження нічого не губить.
  | 'bake'
  | '_meta';
