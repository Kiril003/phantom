 ...
 * відкрити їх можна лише з цим описом, а він їде запечатаним між вузлами.
 */
export interface SecureMedia {
  /** Імʼя, яке дала людина. Їде в тілі повідомлення — обидва вузли його бачать. */
  name: string;
...
export interface Chat {
  /**
   * Дорога до цієї людини ПРЯМО ЗАРАЗ, як її бачить вузол: `direct`, `relay`,
   * `cloud`, `self` — або порожньо, коли дороги немає жодної.
   *
   * Порожнє значення означає, що лист ляже в чергу й лежатиме. Людина мусить
   * знати це ДО того, як напише, а не побачити «у черзі» й гадати.
   */
  roadAhead?: string;
  id: string;
  title: string;
